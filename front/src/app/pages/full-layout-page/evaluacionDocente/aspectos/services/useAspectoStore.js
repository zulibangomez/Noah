...
export const useAspectoStore = () => {
  ...
  ///accion
  const setActivarEvent = (aspectoEvent) => {
    ///el evento creado
  ...
  const startAspecto = async (aspectoEvent) => {
    try {
      ...
      if (aspectoEvent.id) {
        ...
        await noahApi.put(
          `/evaDocente/updateAspecto/${aspectoEvent.id}`,
          aspectoEvent
        );
        ...
        const response = await noahApi.post(
          "/evaDocente/createAspecto",
          aspectoEvent
        );
  ...
  const deleteEventoAspecto = async (id) => {
    try {
      ...
      if (isNaN(aspectoId) || aspectoId <= 0) {
        // Verificar que no sea NaN ni menor o igual a cero
        throw new Error("El ID debe ser un número entero positivo válido.");
      }
      const response = await noahApi.delete(
        `/evaDocente/deleteAspecto/${aspectoId}`
      );
